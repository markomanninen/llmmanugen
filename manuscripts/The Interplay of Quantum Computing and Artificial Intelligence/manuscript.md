...
Entanglement is another seminal concept. When two qubits are entangled, a change in the state of one qubit instantaneously influences the state of its entangled partner, regardless of the distance that separates them. Mathematically, an entangled state can be represented as
...
This property has been leveraged in quantum algorithms to solve problems such as quantum key distribution (Bennett and Brassard, 1984).
...
Reinforcement learning is steered by algorithms such as Q-learning and Deep Q Networks (DQN). Q-learning is modeled as $ Q(s, a) \leftarrow (1 - \alpha) Q(s, a) + \alpha [r + \gamma \max_{a'} Q(s', a')] $, where $ \alpha, \gamma $ are learning and discount factors, respectively. These algorithms are pivotal in applications ranging from game theory to robotics, as demonstrated by the AlphaGo program by DeepMind.
